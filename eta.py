...
class Object:
    ...
    def draw(self):
        ...
        if (libtcod.map_is_in_fov(fov_map, self.x, self.y) or
            ...
            libtcod.console_put_char(con, self.x, self.y, self.char.strip(), libtcod.BKGND_NONE)
...
def render_bar(x, y, total_width, name, value, maximum, bar_color, back_color):
    ...
    libtcod.console_set_default_foreground(panel, bar_color)
    if bar_width > 0:
    ...
    if fov_recompute:
        ...
        for y in range(MAP_HEIGHT):
            for x in range(MAP_WIDTH):
                ...
                wall = map[x][y].block_sight #some item thing
                if not visible:
...
libtcod.console_init_root(SCREEN_WIDTH, SCREEN_HEIGHT, 'eta', False)
